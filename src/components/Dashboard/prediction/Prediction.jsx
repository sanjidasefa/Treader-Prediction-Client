...
const updateGameState = () => {
 ...
 const remain = 30 - (diffInSeconds % 30);
  ...
  useEffect(() => {
    updateGameState();
    const timer = setInterval(updateGameState, 1000);
  ...
  <div className={`${showWeb ? (isExpanded ? "hidden" : "h-[40vh]") : "flex-1"} transition-all duration-300 p-3 space-y-3 overflow-y-auto`}>
    ...
    <div className="flex justify-between items-center bg-white/5 backdrop-blur-md p-3 rounded-xl border border-white/10 shadow-lg">
      <div className="flex items-center gap-3">
        <img src={introimg} className="w-8 h-8 rounded-full border border-cyan-400/40" />
    ...
    <div className="grid grid-cols-2 gap-3">
      ...
      <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-4 shadow-lg relative overflow-hidden">
        ...
        <h1 className={`text-3xl md:text-4xl font-black mt-1 ${
          currentData.size === "Big" ? "text-orange-400" : "text-cyan-400"
        }`}>
          {currentData.size}
        </h1>

        <p className="text-xs font-mono mt-2 text-white/70 break-all">
          P-{fullPeriodId}
        </p>
        ...
        {historyData.map((item, i) => (
          <div key={i} className="flex flex-col items-center bg-white/5 p-2 rounded-lg border border-white/5">
            <span className={`text-sm font-bold ${
              item.size === "Big" ? "text-orange-400" : "text-cyan-400"
            }`}>
              {item.number}
            </span>
  ...
  <AnimatePresence>
    {showWeb && (
      <motion.div
        ...
        animate={{ y: 0 }}
        exit={{ y: "100%" }}
        className={`fixed bottom-0 left-0 w-full bg-white z-50 rounded-t-2xl overflow-hidden ${
      ...
      >
        <div className="bg-black text-white p-3 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => setShowWeb(false)}>
              <X size={18}/>
            </button>
          </div>
        </div>
