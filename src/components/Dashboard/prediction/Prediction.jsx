        ...
        <div className={`flex-grow space-y-3 ${showWeb ? 'lg:w-[40%]' : 'w-full'}`}>
          ...
          <div className="flex justify-between items-center bg-white/5 p-2 px-3 rounded-lg border border-white/10">
            <div className="flex items-center gap-2">
              ...
              <div className="leading-tight">
                <h2 className="text-[9px] font-bold text-cyan-500 uppercase">RS ALGO V4</h2>
                <span className="text-[7px] text-white/30 font-mono uppercase">Status: Live</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 gap-3">
            ...
            <div className="bg-[#0a0a0a] rounded-xl p-4 border border-white/10 relative overflow-hidden">
              ...
              <p className="text-[7px] font-bold text-white/40 tracking-widest mb-1 uppercase">Prediction</p>
              <h1 className={`text-3xl sm:text-5xl font-black italic ${isBig ? 'text-orange-500' : 'text-cyan-400'}`}>
                {data.prediction}
              </h1>
              <div className="mt-2 flex items-center gap-1.5">
                <span className="text-[7px] text-white/20 font-mono">P-{data.period}</span>
            ...
            <div className="bg-white/5 border border-white/10 p-4 rounded-xl flex flex-col justify-center text-center">
              <p className="text-[7px] font-bold text-cyan-500 tracking-widest uppercase">Target No.</p>
              <h3 className="text-3xl sm:text-5xl font-black text-white">
                {data.resultNumber?.toString().padStart(2, '0')}
              </h3>
              <div className="mt-2 flex justify-between text-[7px] font-mono text-white/40 uppercase">
          ...
          <div className="bg-white/5 border border-white/10 p-3 rounded-xl">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-[8px] font-bold text-white/30 uppercase tracking-widest">Recent</h3>
            </div>
              ...
              {data.lastUpdates?.slice(0, 10).map((item, idx) => {
                ...
                return (
                  <div key={idx} className="flex flex-col items-center py-1.5 bg-black/40 rounded-md border border-white/5">
                    <span className={`text-xs font-bold font-mono ${itemIsBig ? 'text-orange-500/80' : 'text-cyan-400/80'}`}>
        ...
        <AnimatePresence>
          {showWeb && (
            <motion.div 
              ...
              className="lg:w-[60%] h-[350px] bg-[#0a0a0a] border border-white/10 rounded-xl overflow-hidden"
            >
              <iframe 
                src="https://www.tradingview.com/chart/" 
                className="w-full h-full border-none grayscale-[0.6] invert-[0.05]"
                title="Market-Feed"
              />
            </motion.div>
          )}
